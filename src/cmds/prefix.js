...
module.exports = {
  ...
  run: async (api, event, args, reply, react) => {
    ...
    reply(response, event, async () => {
      try {
        // Fetch the image and save it to the local file system
        ...
        writer.on('error', (err) => {
          ...
          reply("An error occurred while fetching the image.", event);
        });

      } catch (err) {
        console.error('Error sharing contact or sending image/GIF:', err);
        reply("An error occurred while sharing contact or sending the image/GIF.", event);
      }
    });
  },
};
